...
export class AppCatalogPage extends BasePage {
  ...
  /**
   * Click the final install button ("Install app" or "Save and install")
   */
  private async clickInstallAppButton(): Promise<void> {
    const installButton = this.page.getByRole('button', { name: /Install app|Save and install/i });
    ...
    await this.smartClick(installButton, 'Install/Save and install button');
  ...
  /**
   * Navigate to app via Custom Apps menu with retry logic.
